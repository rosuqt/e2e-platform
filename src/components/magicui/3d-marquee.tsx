...
export function Marquee3D({ reviews }: { reviews?: Array<{ img: string; name: string; username: string; body: string }> }) {
  ...
  const thirdRow = data.slice(0, Math.ceil(data.length / 2));
  const fourthRow = data.slice(Math.ceil(data.length / 2));
