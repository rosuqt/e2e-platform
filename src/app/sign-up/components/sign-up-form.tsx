...
export default function SignUpForm() {
  ...
  const isStepValid = () => {
    if (currentStep === 1) {
      return Object.keys(personalDetailsErrors).every((key) => !personalDetailsErrors[key].trim())
    }
    if (currentStep === 2) {
      return Object.keys(companyErrors).every((key) => !companyErrors[key].trim())
    }
    if (currentStep === 3) {
      return formData.verificationDetails.termsAccepted
    }
    return true
  }
  ...
  return (
    <Card className="w-full max-w-4xl shadow-xl border-2 border-blue-200 bg-white relative overflow-hidden">
      <CardHeader className="relative z-10">
        <motion.h1
          className="text-3xl font-bold text-center bg-gradient-to-r from-blue-500 to-sky-400 text-transparent bg-clip-text mt-5"
      ...
      <CardContent
        className={`pt-6 relative z-10 ${showSuccess ? "h-auto" : ""}`}
        style={showSuccess ? undefined : { height: "400px", overflowY: "auto" }}
      >
        {showSuccess ? (
          <SuccessPage />
        ) : currentStep === 1 ? (
      ...
      <CardFooter className="flex justify-between pb-6 relative z-10">
        {!showSuccess && (
          <>
