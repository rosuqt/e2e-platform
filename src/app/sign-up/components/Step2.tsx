...
export default function Step2({ setCurrentStep }: { setCurrentStep: (step: number) => void }) {
...
const [lastAddedBranch, setLastAddedBranch] = useState<DropdownOption | null>(null);
...
const handleSaveBranch = () => {
  ...
  const [companies, setCompanies] = useState<DropdownOption[]>([
    { id: "kemly-enterprises", name: "Kemly Enterprises", logo: "https://i.pinimg.com/736x/d5/df/38/d5df383773ca1bc126f30096231c285f.jpg" },
    { id: "create-new", name: "+ Create New" }
  ]);



  const [lastAddedCompany, setLastAddedCompany] = useState<DropdownOption | null>(null);

  const handleChange = (field: string, value: DropdownOption | string | null) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [field]: field === "company" ? (value as DropdownOption) : typeof value === "string" ? value : value?.name || "",
    }));
  };
  ...
  return (
    <div className="mt-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Autocomplete suggestions={companyRoles} placeholder="Company Role" onChange={(value) => handleChange("companyRole", value)} />
        <Autocomplete suggestions={jobTitles} placeholder="Job Title" onChange={(value) => handleChange("jobTitle", value)} />
        <input
          type="email"
          name="companyEmail"
