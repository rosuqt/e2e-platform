...
export default function AvailabilityModal({ open, onClose, initial, onSave }: AvailabilityModalProps) {
  ...
  useEffect(() => {
    if (initial) {
      setDays(Array.isArray(initial.days) ? initial.days : ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]);
      setStart(initial.start || defaultTimes.start);
      setEnd(initial.end || defaultTimes.end);
      setTimezone(initial.timezone || defaultTimes.timezone);
    }
  }, [initial, open]);

  const handleSave = async () => {
    const hours: BusinessHours = { days, start, end, timezone };
    if (onSave) {
      await onSave(hours);
    } else {
