          ...
          const applicantsWithDetails = await Promise.all(
            data.applicants.map(async (app: Record<string, unknown>, idx: number): Promise<Applicant> => {
              ...
              return {
                ...
                profile_image_url: app.profile_image_url as string | undefined,
                course: course,
                        ...
                        const percent = analytics?.totalApplicantsThisMonth !== undefined && analytics?.totalApplicantsLastMonth !== undefined
                          ? Math.min(100, Math.round(
                            ((analytics.totalApplicantsThisMonth - analytics.totalApplicantsLastMonth) /
                              Math.max(analytics.totalApplicantsLastMonth, 1)) * 100
                          ))
                          : 0
                        if (percent === 0) {
                          return (
            ...
            <Card className="overflow-hidden border-blue-200 bg-gradient-to-br from-white to-blue-50 h-full">
              ...
              <CardContent className="min-h-[48px]">
                {analyticsLoading ? (
                  ...
                  <div className="flex items-center text-sm font-medium text-green-600">
                    <ArrowUpRight className="h-4 w-4 mr-1" />
                    {analytics?.interviewsScheduled === 1
                      ? "1 scheduled for tomorrow"
                      : `${analytics?.interviewsScheduled ?? 0} scheduled for tomorrow`}
                  </div>
                )}
              </CardContent>
              <div className="h-1 bg-gradient-to-r from-blue-400 to-blue-600"></div>
