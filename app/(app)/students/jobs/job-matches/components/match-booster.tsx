  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setSkills(Array.isArray(json.skills) ? json.skills.map((s: any) => s.name) : null)
        }
      } catch (e) {
...
const steps = ["Intro", "Skills", "Learning", "Done"] as const
type Step = 0 | 1 | 2 | 3
...
export function MatchBoosterPopup({
  ...
  useEffect(() => {
    async function fetchStudentSkillProgress() {
      ...
      if (Array.isArray(json.progress)) {
        const skillIds = json.progress.map((row: any) => row.skill_id)
  ...
  const effectiveTopSkills =
    (topSkills && topSkills.length > 0  
      ? topSkills
      : serverSkills && serverSkills.length > 0
      ? serverSkills
      : []) ?? []
  ...
  const handleMarkCompleted = (skillName: string, resourceTitle: string) => {
    setResourceCompletion((prev) => {
      const key = `${skillName}-${resourceTitle}`
      const updated = { ...prev }
  ...
  const completedSkills = selectedSkills.filter((s) => {
    const resourcesForSkill = effectiveResources.filter((r) => r.name === s.name);
    const easyCompleted = resourcesForSkill.some(
      (r) => r.level === "Easy" && resourceCompletion[`${r.skill_id}-${r.title}`]
    );
    const mediumCompleted = resourcesForSkill.some(
      (r) => r.level === "Medium" && resourceCompletion[`${r.skill_id}-${r.title}`]
    );
    const hardCompleted = resourcesForSkill.some(
      (r) => r.level === "Hard" && resourceCompletion[`${r.skill_id}-${r.title}`]
    );
    return easyCompleted && mediumCompleted && hardCompleted;
  }).length

  const totalRequiredResources = selectedSkills.length * 3
  const completedResources = selectedSkills.reduce((acc, s) => {
    const resourcesForSkill = effectiveResources.filter((r) => r.name === s.name)
    return (
      acc +
      resourcesForSkill.filter(
        (r) =>
          ...
          resourceCompletion[`${r.skill_id}-${r.title}`]
      ).length
    )
  }, 0)
  ...
  function handleSaveProgress() {
    ...
    const progressData = selectedSkillObjs.map(skill => {
            ...
            try {
              ...
              if (Array.isArray(parsed.completed_levels)) {
                parsed.completed_levels.forEach((level: string) => {
                  completion[`${row.skill_id}-${level}`] = true
                })
              }
            } catch {}
          }
  ...
  }, [session?.user?.studentId, open])

  const hasUnsavedProgress = useMemo(() => {
    return Object.entries(resourceCompletion).some(
      ([key, val]) => val && !dbResourceCompletion[key]
    )
  }, [resourceCompletion, dbResourceCompletion])
          ...
          >
            ...
            >
              ...
              >
                ...
                <div className={step === 3 ? "relative z-10" : ""}>
                            ...
                            <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-blue-50 border border-blue-100">
                              <div className="flex flex-col">
                                ...
                                <span className="text-sm font-semibold text-blue-700">
                                  {completedSkills} / {selectedSkills.length || 0}
                                </span>
                              </div>
                          ...
                          {resourcesLoading ? (
                            ...
                            <>
                              ...
                              {skillResources.map(({ skill, resource }, idx) => {
                                ...
                                function handleMarkCompleted(skill_id: string, title: string) {
                                  setResourceCompletion((prev) => {
                                    const key = `${skill_id}-${title}`;
                                    const updated = { ...prev };
                                    updated[key] = !prev[key];
                                    return updated;
                                  });
                    ...
                    {step === 3 && (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <button
                            onClick={onClose}
                            className="w-full rounded-full bg-cyan-400/50 text-white text-sm font-medium py-2.5 hover:bg-cyan-500 border-2 border-cyan-500
