...
export default function DTRDailyLog({ onClose, onSubmit }: DTRDailyLogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For now, pass as JSON. In production, you might use FormData for file upload
      onSubmit({
        date: formData.date,
        description: formData.description,
        hours: Number.parseFloat(formData.hours),
        imageProof: imagePreview, // Base64 for demo
      })
    } finally {
      setLoading(false)
    }
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-sky-500 px-6 py-6 flex items-center justify-between rounded-t-2xl">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {/* Hours */}
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-2">Hours Completed *</label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">hrs</span>
              </div>
            </div>
            ...
            {/* Image Proof */}
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-2">Proof of Work (Image)</label>
              <div
            ...
            <div className="flex gap-3 pt-4">
              ...
              <motion.button
                ...
                type="submit"
                disabled={loading}
                className="flex-1 px-6 py-3 rounded-lg bg-gradient-to-r from-blue-600 to-sky-500 text-white font-semibold hover:shadow-lg transition-all disabled:opacity-70"
              >
