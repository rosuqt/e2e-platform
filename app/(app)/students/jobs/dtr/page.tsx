...
export default function DTRPage() {
  ...
  const fetchJobInfo = async () => {
    try {
      const res = await fetch("/api/dtr/getJobInfo")
      const data = await res.json()
      if (data.jobInfo) {
        setJobInfo(data.jobInfo)
        fetchLogs(data.jobInfo.id)
      }
      setLoading(false)
    } catch (error) {
  ...
  const fetchLogs = async (jobId: string) => {
    try {
      const res = await fetch(`/api/dtr/getLogs?jobId=${jobId}`)
      const data = await res.json()
      setLogs(data.logs || [])
    } catch (error) {
  ...
  const handleJobSetup = async (data: any) => {
    try {
      const res = await fetch("/api/dtr/setJobInfo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      })
      const result = await res.json()
      setJobInfo(result.jobInfo)
      setRefreshKey((prev) => prev + 1)
    } catch (error) {
      console.error("Error setting job info:", error)
    }
  }

  const handleAddLog = async (logData: any) => {
    try {
      const res = await fetch("/api/dtr/addLog", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...logData,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {!jobInfo ? (
          ...
          <div className="space-y-6">
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/80 text-lg">{jobInfo.company}</p>
                    <div className="mt-4 flex items-center gap-6">
                      <div>
                        <p className="text-white/60 text-sm">Total Required Hours</p>
                        <p className="text-white text-2xl font-bold">{jobInfo.totalHours} hrs</p>
                      </div>
                      <div>
            ...
            {/* Dashboard Overview */}
            <DTRDashboard jobInfo={jobInfo} logs={logs} refreshKey={refreshKey} />

            {/* Daily Logs Section */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-blue-100/50 p-6"
            >
