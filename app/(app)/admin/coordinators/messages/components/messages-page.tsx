...
export default function MessageInterface() {
  ...
  const getConversations = async () => {
    async function fetchConversations() {
      try {
        const res = await fetch("/api/admin/messages-events/get-contacts", { method: "GET" });
        const data = await res.json();
  ...
  const handleSendMessage = async (convId: string) => {
    ...
    try {
      const res = await fetch("/api/admin/messages-events/get-contacts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
          ...
          {conversations.filter(adacc => adacc.name.toLowerCase().includes(search.toLowerCase())).map((conversation) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500">{conversation.role === "Admin" ? "Coordinator" : conversation.role}</p>        
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="w-full text-sm">
            ...
            {isOpenE && (
              ...
              {employers.filter(adacc => adacc.first_name.toLowerCase().includes(search)).map((e) => (
                <div
                  key={e.id}
                  className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-slate-50 border-l-4`}
